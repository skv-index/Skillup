...
import { currentUser, submitChallengeAi } from '../services/part2-store';
import '../part2.css';
...
export function ChallengeSubmissionPage() {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const evaluation = await submitChallengeAi(challenge.id, solution.trim(), checks, currentUser());
  ...
  return (
    <div>
      ...
      <div className="page-head">
        ...
        <p>Performance Evaluation · {aiAvailable() ? 'AI-graded' : 'auto-graded'} against the rubric · +{challenge.points} pts</p>
      </div>
      ...
      <div className="grid grid--2">
        ...
        <Card title="Self-check rubric" subtitle="Honest checks raise your completeness score">
          ...
          <Alert variant="info">
            {aiAvailable() ? (
              <>AI grading reads your solution against the rubric and scores completeness (60) + substance (30) + prior assessment bonus (10). Pass at 60+.</>
            ) : (
              <>Scoring: checklist completeness (60) + solution substance (30) + prior assessment bonus (10). Pass at 60+.</>
            )}
          </Alert>
          {aiAvailable() && (
            <div className="mt-4">
              <Badge variant="primary">✨ AI grader will read your solution for evidence — claimed checks alone don't score.</Badge>
            </div>
          )}
        </Card>
      </div>
